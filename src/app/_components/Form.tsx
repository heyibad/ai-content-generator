...
const Form = ({ type }: { type: "login" | "signup" }) => {
  ...
  const validateForm = () => {
    ...
    if (type === "signup" && form.username && /[^a-zA-Z0-9]/.test(form.username)) {
  ...
  return (
    <form className="text-[#18181B]" onSubmit={onSubmitHandle} method="POST">
      <input
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? "Loading..." : type === "signup" ? <User size={18} /> : <LockOpen size={18} />}
        <span className="ml-3">
